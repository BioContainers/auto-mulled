...
if [ "$TRAVIS_SECURE_ENV_VARS" = "true" -a "$TRAVIS_BRANCH" = "master" -a "$TRAVIS_PULL_REQUEST" = "false" ]; then
	...
	for i in $AFFECTED; do
		echo -n "* $i"
		curl --silent -H Authorization:Bearer\ $QUAY_TOKEN https://quay.io/api/v1/repository -d '{"namespace":"'$NAMESPACE'", "visibility":"public","repository":"'$i'", "description":"Auto-generated by [mulled/auto-mulled](https://github.com/mulled/auto-mulled)"}' || echo -n " FAILED!"
		echo
	done
